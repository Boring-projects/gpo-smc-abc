        ...
        if tt != 0:
            ...
            if (classSMC.condFilterInternal == 1):
                ...
                if (classSMC.ancestorSamplingInternal == 1):
                    ...
                    if (classSMC.filterTypeInternal == "bootstrap"):
                        tmp = sys.evaluateState(
                            classSMC.condPath[0, tt], p[:, tt], tt)
                    elif (classSMC.filterTypeInternal == "fullyadapted"):
                        tmp = sys.evaluateStateFA(
                            classSMC.condPath[0, tt], p[:, tt], tt)
                    ...
                    # Ancestor sampling, normalize weights and sample ancestor
                    # index
                    tmp = tmp / np.nansum(tmp)
                    classSMC.tmp = tmp
                    a[classSMC.nPart - 1,
                        tt] = np.random.choice(classSMC.nPart, 1, p=tmp)
